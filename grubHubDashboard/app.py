...
def getDataForBarChart():
    results = db.session.query(GrubHubDashboard.establishment, func.sum(GrubHubDashboard.total).label('totalSorted')).group_by(GrubHubDashboard.establishment).order_by(('totalSorted').desc()).all()
