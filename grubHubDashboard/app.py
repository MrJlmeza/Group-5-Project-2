...
def getDataForBarChart():
    results = db.session.query(GrubHubDashboard.establishment, func.sum(GrubHubDashboard.total)).group_by(GrubHubDashboard.establishment).order_by((GrubHubDashboard.total).desc()).all()
